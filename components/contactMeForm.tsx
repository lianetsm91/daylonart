...
const ContactMeForm = () => {
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        let nameErrors = `${noSpecialCharacters(formValues.get('name')?.value)} ${maxLength(formValues.get('name')?.value, 50)}`.trim();
        let emailErrors = `${invalidEmail(formValues.get('email')?.value)} ${maxLength(formValues.get('email')?.value, 74)}`.trim();
    ...
    const onBlur = (field: string) => {
        if (!formValues.get(field)?.value) {
            setFormValues(new Map(formValues.set(field, {
